...
export default function Meydan() {
  ...
  const openChatWith = async (otherUser) => {
    ...
    // Sohbet satırını Supabase'e göndermeden önce gerçekten geçerli bir
    // oturum olduğunu doğrula. auth.uid() null gelirse RLS policy'si
    // isteği zaten reddeder (42501); burada kullanıcıya daha anlaşılır
    // bir mesaj göstermek ve gereksiz isteği önlemek için erken çıkıyoruz.
    const session = await api.getCurrentSession();
    if (!session?.user?.id) {
      setChatError("Sohbet başlatmak için giriş yapmış olmalısın. Lütfen tekrar giriş yap.");
      return;
    }

    try {
      const conversationId = await api.findOrCreateConversation(session.user.id, otherUser.id);
      await refreshChats();
      ...
      if (err?.code === "42501") {
        setChatError(
          "Sohbet oluşturma izniniz yok. Supabase tarafında conversations / conversation_members " +
            "tablolarının RLS policy'lerini kontrol edin (bkz. supabase/migrations)."
        );
      } else {
        setChatError(err?.message || "Sohbet açılamadı, tekrar dene.");
      }
    }
  };
  ...
  return (
    <div className="flex min-h-screen" style={{ background: COLORS.bg, color: COLORS.ivory, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="flex-1 pb-16 md:pb-0">
        {chatError && (
          <div
            className="mx-4 mt-4 px-4 py-2 rounded-md text-sm flex items-center justify-between gap-3"
            style={{ background: "#3A1F24", border: "1px solid #7A2E38", color: "#F3EDE0" }}
          >
            <span>{chatError}</span>
            <button
